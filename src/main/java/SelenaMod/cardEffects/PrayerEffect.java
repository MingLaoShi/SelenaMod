...
public class PrayerEffect extends AbstractCardEffect {
    public PrayerEffect(String id, int amount) {
        super(id);
        this.data.setAmount(amount);
        PowerStrings toneStrings = CardCrawlGame.languagePack.getPowerStrings(ModHelper.makeID(PrayerEffect.class.getSimpleName()));
    ...
    public AbstractGameAction trigger(AbstractCreature target) {
        return new ApplyPowerAction(AbstractDungeon.player, AbstractDungeon.player, new MantraPower(AbstractDungeon.player, this.data.amount));
    }
